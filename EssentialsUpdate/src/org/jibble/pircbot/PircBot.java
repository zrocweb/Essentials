...
/**
 * PircBot is a Java framework for writing IRC bots quickly and easily. <p> It provides an event-driven architecture to
 * handle common IRC events, flood protection, DCC support, ident support, and more. The comprehensive logfile format is
 * suitable for use with pisg to generate channel statistics. <p> Methods of the PircBot class can be called to send
 * events to the IRC server that it connects to. For example, calling the sendMessage method will send a message to a
 * channel or user on the IRC server. Multiple servers can be supported using multiple instances of PircBot. <p> To
 * perform an action when the PircBot receives a normal message from the IRC server, you would override the onMessage
 * method defined in the PircBot class. All on<i>XYZ</i> methods in the PircBot class are automatically called when the
 * event <i>XYZ</i> happens, so you would override these if you wish to do something when it does happen. <p> Some event
 * methods, such as onPing, should only really perform a specific function (i.e. respond to a PING from the server). For
 * your convenience, such methods are already correctly implemented in the PircBot and should not normally need to be
 * overridden. Please read the full documentation for each method to see which ones are already implemented by the
 * PircBot class. <p> Please visit the PircBot homepage at <a
 * href="http://www.jibble.org/pircbot.php">http://www.jibble.org/pircbot.php</a> for full revision history, a beginners
 * guide to creating your first PircBot and a list of some existing Java IRC bots and clients that use the PircBot
 * framework.
 *
...
{
	/**
	 * The definitive version number of this release of PircBot. (Note: Change this before automatically building
	 * releases)
	 */
	...
	/**
	 * Constructs a PircBot with the default settings. Your own constructors in classes which extend the PircBot
	 * abstract class should be responsible for changing the default settings if required.
	 */
	public PircBot()
	{
	}
	 ...
	 * connected to an IRC server, the onDisconnect() will be called as soon as the disconnection is made by the server.
	 *
	...
	/**
	 * When you connect to a server and your nick is already in use and this is set to true, a new nick will be
	 * automatically chosen. This is done by adding numbers to the end of the nick until an available nick is found.
	 *
	 * @param autoNickChange Set to true if you want automatic nick changes during connection.
	 */
	 ...
	 * queue and sent at the earliest possible opportunity. <p> Some examples: -
	...
	/**
	 * Sends a CTCP command to a channel or user. (Client to client protocol). Examples of such commands are "PING
	 * <number>", "FINGER", "VERSION", etc. For example, if you wish to request the version of a user called "Dave",
	 * then you would call
	 * <code>sendCTCPCommand("Dave", "VERSION");</code>. The type of response to such commands is largely dependant on
	...
	/**
	 * Identify the bot with NickServ, supplying the appropriate password. Some IRC Networks (such as freenode) require
	 * users to <i>register</i> and <i>identify</i> with NickServ before they are able to send private messages to other
	 * users, thus reducing the amount of spam. If you are using an IRC network where this kind of policy is enforced,
	 * you will need to make your bot <i>identify</i> itself to NickServ before you can send private messages. Assuming
	 * you have already registered your bot's nick with NickServ, this method can be used to <i>identify</i> with the
	 * supplied password. It usually makes sense to identify with NickServ immediately after connecting to a server. <p>
	 * This method issues a raw NICKSERV command to the server, and is therefore safer than the alternative approach of
	 * sending a private message to NickServ. The latter approach is considered dangerous, as it may cause you to
	 ...
	 * @param mode The new mode to apply to the channel. This may include zero or more arguments if necessary.
	 *
	 ...
	 * operator status if the topic is protected.
	 *
	 * @param channel The channel on which to perform the mode change.
	 ...
	 * Issues a request for a list of all channels on the IRC server. When the PircBot receives information for each
	 * channel, it will call the onChannelInfo method, which you will need to override if you want it to do anything
	 * useful. <p> Some IRC servers support certain parameters for LIST requests. One example is a parameter of ">10" to
	 * list only those channels that have more than 10 users in them. Whether these parameters are supported or not will
	 * depend on the IRC server software.
	 *
	...
	/**
	 * Adds a line to the log. This log is currently output to the standard output and is in the correct format for use
	 * by tools such as pisg, the Perl IRC Statistics Generator. You may override this method if you wish to do
	 * something else with log entries. Each line in the log begins with a number which represents the logging time (as
	 * the number of milliseconds since the epoch). This timestamp and the following log entry are separated by a single
	 * space character, " ". Outgoing messages are distinguishable by a log entry that has ">>>" immediately following
	 * the space character after the timestamp. DCC events use "+++" and warnings about unhandled Exceptions and Errors
	 * use "###". <p> This implementation of the method will only cause log entries to be output if the PircBot has had
	 * its verbose mode turned on by calling setVerbose(true);
	 *
	 ...
	 * the PircBot. This method is protected and only called by the InputThread for this instance. <p> This method may
	 * not be overridden!
	 *
	...
	/**
	 * This method is called once the PircBot has successfully connected to the IRC server. <p> The implementation of
	 * this method in the PircBot abstract class performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.6
	 */
	 ...
	 * PircBot quits from the server, or if the connection is unexpectedly lost. <p> Disconnection from the IRC server
	 * is detected immediately if either we or the server close the connection normally. If the connection to the server
	 * is lost, but neither we nor the server have explicitly closed the connection, then it may take a few minutes to
	 * detect (this is commonly referred to as a "ping timeout"). <p> If you wish to get your IRC bot to automatically
	 * rejoin a server after the connection has been lost, then this is probably the ideal method to override to
	 * implement such functionality. <p> The implementation of this method in the PircBot abstract class performs no
	 ...
	 * method. <p> Note that this method is private and should not appear in any of the javadoc generated documenation.
	 *
	...
	/**
	 * This method is called when we receive a numeric response from the IRC server. <p> Numerics in the range from 001
	 * to 099 are used for client-server connections only and should never travel between servers. Replies generated in
	 * response to commands are found in the range from 200 to 399. Error replies are found in the range from 400 to
	 * 599. <p> For example, we can use this method to discover the topic of a channel when we join it. If we join the
	 * channel #test which has a topic of &quot;I am King of Test&quot; then the response will be &quot;
	 * <code>PircBot #test :I Am King of Test</code>&quot; with a code of 332 to signify that this is a topic. (This is
	 * just an example - note that overriding the
	 * <code>onTopic</code> method is an easier way of finding the topic for a channel). Check the IRC RFC for the full
	 * list of other command response codes. <p> PircBot implements the interface ReplyConstants, which contains
	 * contstants that you may find useful here. <p> The implementation of this method in the PircBot abstract class
	...
	/**
	 * This method is called when we receive a user list from the server after joining a channel. <p> Shortly after
	 * joining a channel, the IRC server sends a list of all users in that channel. The PircBot collects this
	 * information and calls this method as soon as it has the full list. <p> To obtain the nick of each user in the
	 * channel, call the getNick() method on each User object in the array. <p> At a later time, you may call the
	 * getUsers method to obtain an up to date list of the users in the channel. <p> The implementation of this method
	 * in the PircBot abstract class performs no actions and may be overridden as required.
	 *
	 * @since PircBot 1.0.0
	 *
	...
	/**
	 * This method is called whenever a message is sent to a channel. <p> The implementation of this method in the
	 * PircBot abstract class performs no actions and may be overridden as required.
	 *
	...
	/**
	 * This method is called whenever a private message is sent to the PircBot. <p> The implementation of this method in
	 * the PircBot abstract class performs no actions and may be overridden as required.
	 *
	 ...
	 * shopping" in most IRC clients. <p> The implementation of this method in the PircBot abstract class performs no
	 * actions and may be overridden as required.
	 *
	...
	/**
	 * This method is called whenever we receive a notice. <p> The implementation of this method in the PircBot abstract
	...
	/**
	 * This method is called whenever someone (possibly us) joins a channel which we are on. <p> The implementation of
	 * this method in the PircBot abstract class performs no actions and may be overridden as required.
	 *
	...
	/**
	 * This method is called whenever someone (possibly us) parts a channel which we are on. <p> The implementation of
	 * this method in the PircBot abstract class performs no actions and may be overridden as required.
	 *
	...
	/**
	 * This method is called whenever someone (possibly us) changes nick on any of the channels that we are on. <p> The
	 * implementation of this method in the PircBot abstract class performs no actions and may be overridden as
	 * required.
	 *
	...
	/**
	 * This method is called whenever someone (possibly us) is kicked from any of the channels that we are in. <p> The
	 * implementation of this method in the PircBot abstract class performs no actions and may be overridden as
	 * required.
	 *
	 ...
	 * was in one of the channels to which we are connected. <p> The implementation of this method in the PircBot
	 ...
	 * topic. <p> The implementation of this method in the PircBot abstract class performs no actions and may be
	 ...
	 * soon as it is received. <p> Note that certain channels, such as those marked as hidden, may not appear in channel
	 * listings. <p> The implementation of this method in the PircBot abstract class performs no actions and may be
	 * overridden as required.
	 *
	 * @param channel The name of the channel.
	 ...
	 * method before finally calling the override-able onMode method. <p> Note that this method is private and is not
	 * intended to appear in the javadoc generated documentation.
	 *
	...
	/**
	 * Called when the mode of a channel is set. <p> You may find it more convenient to decode the meaning of the mode
	 * string by overriding the onOp, onDeOp, onVoice, onDeVoice, onChannelKey, onDeChannelKey, onChannelLimit,
	 * onDeChannelLimit, onChannelBan or onDeChannelBan methods as appropriate. <p> The implementation of this method in
	 * the PircBot abstract class performs no actions and may be overridden as required.
	 *
	...
	/**
	 * Called when the mode of a user is set. <p> The implementation of this method in the PircBot abstract class
	 * performs no actions and may be overridden as required.
	 *
	 * @since PircBot 1.2.0
	 *
	...
	/**
	 * Called when a user (possibly us) gets granted operator status for a channel. <p> This is a type of mode change
	 * and is also passed to the onMode method in the PircBot class. <p> The implementation of this method in the
	 * PircBot abstract class performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 *
	...
	/**
	 * Called when a user (possibly us) gets operator status taken away. <p> This is a type of mode change and is also
	 * passed to the onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract
	 * class performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 *
	...
	/**
	 * Called when a user (possibly us) gets voice status granted in a channel. <p> This is a type of mode change and is
	 * also passed to the onMode method in the PircBot class. <p> The implementation of this method in the PircBot
	 * abstract class performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 *
	...
	/**
	 * Called when a user (possibly us) gets voice status removed. <p> This is a type of mode change and is also passed
	 * to the onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract class
	 * performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 *
	 ...
	 * they know the key. Channel keys are sometimes referred to as passwords. <p> This is a type of mode change and is
	 * also passed to the onMode method in the PircBot class. <p> The implementation of this method in the PircBot
	 * abstract class performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	...
	/**
	 * Called when a channel key is removed. <p> This is a type of mode change and is also passed to the onMode method
	 * in the PircBot class. <p> The implementation of this method in the PircBot abstract class performs no actions and
	...
	/**
	 * Called when a user limit is set for a channel. The number of users in the channel cannot exceed this limit. <p>
	 * This is a type of mode change and is also passed to the onMode method in the PircBot class. <p> The
	 * implementation of this method in the PircBot abstract class performs no actions and may be overridden as
	 * required.
	 *
	 * @since PircBot 0.9.5
	...
	/**
	 * Called when the user limit is removed for a channel. <p> This is a type of mode change and is also passed to the
	 * onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract class performs
	 ...
	 * being kicked :-) <p> This is a type of mode change and is also passed to the onMode method in the PircBot class.
	 * <p> The implementation of this method in the PircBot abstract class performs no actions and may be overridden as
	 * required.
	 *
	 * @since PircBot 0.9.5
	 *
	...
	/**
	 * Called when a hostmask ban is removed from a channel. <p> This is a type of mode change and is also passed to the
	 * onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract class performs
	 * no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 ...
	 * may change the topic. <p> This is a type of mode change and is also passed to the onMode method in the PircBot
	 * class. <p> The implementation of this method in the PircBot abstract class performs no actions and may be
	 * overridden as required.
	 *
	 * @since PircBot 0.9.5
	 *
	...
	/**
	 * Called when topic protection is removed for a channel. <p> This is a type of mode change and is also passed to
	 * the onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract class
	 * performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 *
	...
	/**
	 * Called when a channel is set to only allow messages from users that are in the channel. <p> This is a type of
	 * mode change and is also passed to the onMode method in the PircBot class. <p> The implementation of this method
	 * in the PircBot abstract class performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 *
	...
	/**
	 * Called when a channel is set to allow messages from any user, even if they are not actually in the channel. <p>
	 * This is a type of mode change and is also passed to the onMode method in the PircBot class. <p> The
	 * implementation of this method in the PircBot abstract class performs no actions and may be overridden as
	 * required.
	 *
	 ...
	 * someone who is already in the channel. <p> This is a type of mode change and is also passed to the onMode method
	 * in the PircBot class. <p> The implementation of this method in the PircBot abstract class performs no actions and
	 * may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	...
	/**
	 * Called when a channel has 'invite only' removed. <p> This is a type of mode change and is also passed to the
	 * onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract class performs
	 * no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 ...
	 * 'voiced' or 'opped' may speak or change their nicks. <p> This is a type of mode change and is also passed to the
	 * onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract class performs
	 * no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	...
	/**
	 * Called when a channel has moderated mode removed. <p> This is a type of mode change and is also passed to the
	 * onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract class performs
	 * no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	...
	/**
	 * Called when a channel is marked as being in private mode. <p> This is a type of mode change and is also passed to
	 * the onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract class
	 * performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	...
	/**
	 * Called when a channel is marked as not being in private mode. <p> This is a type of mode change and is also
	 * passed to the onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract
	 * class performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 ...
	 * listing. <p> This is a type of mode change and is also passed to the onMode method in the PircBot class. <p> The
	 * implementation of this method in the PircBot abstract class performs no actions and may be overridden as
	 * required.
	 *
	 * @since PircBot 0.9.5
	...
	/**
	 * Called when a channel has 'secret' mode removed. <p> This is a type of mode change and is also passed to the
	 * onMode method in the PircBot class. <p> The implementation of this method in the PircBot abstract class performs
	 * no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	...
	/**
	 * Called when we are invited to a channel by a user. <p> The implementation of this method in the PircBot abstract
	 * class performs no actions and may be overridden as required.
	 *
	 * @since PircBot 0.9.5
	 *
	...
	/**
	 * This method used to be called when a DCC SEND request was sent to the PircBot. Please use the
	 * onIncomingFileTransfer method to receive files, as it has better functionality and supports resuming.
	 *
	...
	/**
	 * This method used to be called when a DCC CHAT request was sent to the PircBot. Please use the
	 * onIncomingChatRequest method to accept chats, as it has better functionality.
	 *
	...
	/**
	 * This method is called whenever we receive a PING request from another user. <p> This abstract implementation
	 * responds correctly, so if you override this method, be sure to either mimic its functionality or to call
	 * super.onPing(...);
	 *
	...
	/**
	 * The actions to perform when a PING request comes from the server. <p> This sends back a correct response, so if
	 * you override this method, be sure to either mimic its functionality or to call super.onServerPing(response);
	 *
	...
	/**
	 * This method is called whenever we receive a TIME request. <p> This abstract implementation responds correctly, so
	 * if you override this method, be sure to either mimic its functionality or to call super.onTime(...);
	 *
	...
	/**
	 * This method is called whenever we receive a FINGER request. <p> This abstract implementation responds correctly,
	 * so if you override this method, be sure to either mimic its functionality or to call super.onFinger(...);
	 *
	 ...
	 * recognise. <p> The implementation of this method in the PircBot abstract class performs no actions and may be
	 * overridden as required.
	 *
	...
	/**
	 * Sets the verbose mode. If verbose mode is set to true, then log entries will be printed to the standard output.
	 * The default value is false and will result in no output. For general development, we strongly recommend setting
	 * the verbose mode to true.
	 *
	...
	/**
	 * Sets the name of the bot, which will be used as its nick when it tries to join an IRC server. This should be set
	 * before joining any servers, otherwise the default nick will be used. You would typically call this method from
	 * the constructor of the class that extends PircBot. <p> The changeNick method should be used if you wish to change
	 ...
	 * the old nick until confirmation of the nick change is received from the server. <p> The nick returned by this
	 * method is maintained only by the PircBot class and is guaranteed to be correct in the context of the IRC server.
	 *
	 * @since PircBot 1.0.0
	 *
	...
	/**
	 * Sets the number of milliseconds to delay between consecutive messages when there are multiple messages waiting in
	 * the outgoing message queue. This has a default value of 1000ms. It is a good idea to stick to this default value,
	 * as it will prevent your bot from spamming servers and facing the subsequent wrath! However, if you do need to
	 * change this delay value (<b>not recommended</b>), then this is the method to use.
	 *
	...
	/**
	 * Returns the number of milliseconds that will be used to separate consecutive messages to the server from the
	...
	/**
	 * Gets the maximum length of any line that is sent via the IRC protocol. The IRC RFC specifies that line lengths,
	 * including the trailing \r\n must not exceed 512 bytes. Hence, there is currently no option to change this value
	 * in PircBot. All lines greater than this length will be truncated before being sent to the IRC server.
	 *
	...
	/**
	 * Gets the number of lines currently waiting in the outgoing message Queue. If this returns 0, then the Queue is
	 * empty and any new message is likely to be sent to the IRC server immediately.
	 *
	 * @since PircBot 0.9.9
	 *
	...
	/**
	 * Returns the name of the last IRC server the PircBot tried to connect to. This does not imply that the connection
	 * attempt to the server was successful (we suggest you look at the onConnect method). A value of null is returned
	 * if the PircBot has never tried to connect to a server.
	 *
	 * @return The name of the last machine we tried to connect to. Returns null if no connection attempts have ever
	 * been made.
	 */
	...
	/**
	 * Returns the port number of the last IRC server that the PircBot tried to connect to. This does not imply that the
	 * connection attempt to the server was successful (we suggest you look at the onConnect method). A value of -1 is
	 * returned if the PircBot has never tried to connect to a server.
	 *
	 * @since PircBot 0.9.9
	 *
	 * @return The port number of the last IRC server we connected to. Returns -1 if no connection attempts have ever
	 * been made.
	 */
	...
	/**
	 * Returns the last password that we used when connecting to an IRC server. This does not imply that the connection
	 * attempt to the server was successful (we suggest you look at the onConnect method). A value of null is returned
	 * if the PircBot has never tried to connect to a server using a password.
	 *
	 * @since PircBot 0.9.9
	 *
	 * @return The last password that we used when connecting to an IRC server. Returns null if we have not previously
	 * connected using a password.
	 */
	...
	/**
	 * Sets the encoding charset to be used when sending or receiving lines from the IRC server. If set to null, then
	 * the platform's default charset is used. You should only use this method if you are trying to send text to an IRC
	 * server in a different charset, e.g. "GB2312" for Chinese encoding. If a PircBot is currently connected to a
	 * server, then it must reconnect before this change takes effect.
	 *
	 * @since PircBot 1.0.4
	 *
	...
	/**
	 * Returns the encoding used to send and receive lines from the IRC server, or null if not set. Use the setEncoding
	...
	/**
	 * Returns the InetAddress used by the PircBot. This can be used to find the I.P. address from which the PircBot is
	...
	/**
	 * Sets the InetAddress to be used when sending DCC chat or file transfers. This can be very useful when you are
	 * running a bot on a machine which is behind a firewall and you need to tell receiving clients to connect to a
	...
	/**
	 * Returns the InetAddress used when sending DCC chat or file transfers. If this is null, the default InetAddress
	 * will be used.
	 *
	 * @since PircBot 1.4.4
	 *
	...
	/**
	 * Returns the set of port numbers to be used when sending a DCC chat or file transfer. This is useful when you are
	 * behind a firewall and need to set up port forwarding. The array of port numbers is traversed in sequence until a
	 * free port is found to listen on. A DCC tranfer will fail if all ports are already in use. If set to null,
	 * <i>any</i> free port number will be used.
	 *
	 * @since PircBot 1.4.4
	 *
	...
	/**
	 * Sets the choice of port numbers that can be used when sending a DCC chat or file transfer. This is useful when
	 * you are behind a firewall and need to set up port forwarding. The array of port numbers is traversed in sequence
	 * until a free port is found to listen on. A DCC tranfer will fail if all ports are already in use. If set to null,
	 * <i>any</i> free port number will be used.
	 *
	 * @since PircBot 1.4.4
	 *
	 * @param ports The set of port numbers that PircBot may use for DCC transfers, or null to let it use any free port
	 * (default).
	 *
	 */
	...
	/**
	 * Returns true if and only if the object being compared is the exact same instance as this PircBot. This may be
	 * useful if you are writing a multiple server IRC bot that uses more than one instance of PircBot.
	 *
	 * @since PircBot 0.9.9
	 *
	...
	/**
	 * Returns the hashCode of this PircBot. This method can be called by hashed collection classes and is useful for
	 * managing multiple instances of PircBots in such collections.
	 *
	 * @since PircBot 0.9.9
	 *
	...
	/**
	 * Returns a String representation of this object. You may find this useful for debugging purposes, particularly if
	 * you are using more than one PircBot instance to achieve multiple server connectivity. The format of this String
	 * may change between different versions of PircBot but is currently something of the form
	 * <code>
	...
	/**
	 * Returns an array of all users in the specified channel. <p> There are some important things to note about this
	 * method:- <ul> <li>This method may not return a full list of users if you call it before the complete nick list
	 * has arrived from the IRC server. </li> <li>If you wish to find out which users are in a channel as soon as you
	 * join it, then you should override the onUserList method instead of calling this method, as the onUserList method
	 * is only called as soon as the full user list has been received. </li> <li>This method will return immediately, as
	 * it does not require any interaction with the IRC server. </li> <li>The bot must be in a channel to be able to
	 * know which users are in it. </li> </ul>
	 *
	 * @since PircBot 1.0.0
	 *
	...
	/**
	 * Disposes of all thread resources used by this PircBot. This may be useful when writing bots or clients that use
	 * multiple servers (and therefore multiple PircBot instances) or when integrating a PircBot with an existing
	 * program. <p> Each PircBot runs its own threads for dispatching messages from its outgoing message queue and
	 * receiving messages from the server. Calling dispose() ensures that these threads are stopped, thus freeing up
	 * system resources and allowing the PircBot object to be garbage collected if there are no other references to it.
	 * <p> Once a PircBot object has been disposed, it should not be used again. Attempting to use a PircBot that has
	 * been disposed may result in unpredictable behaviour.
	 *
	 * @since 1.2.2
	 */
